...
async def _send_campaign_background(campaign_id: str):
    """Kick off campaign sending via the Celery scheduling pipeline."""
    from app.tasks.campaign_tasks import schedule_campaign_sends_task

    # Delegate to Celery — handles daily limits, jitter, domain rotation, throttling
    result = schedule_campaign_sends_task.delay(campaign_id)
    logger.info("Campaign %s send scheduled via Celery task %s", campaign_id, result.id)
